...
func TestResolveSettings(t *testing.T) {
	...
	t.Run("max scale accepts the int32 boundary", func(t *testing.T) {
		got, err := ResolveSettings(map[string]string{
			ClassAnnotation:    KEDAClass,
			MaxScaleAnnotation: strconv.FormatInt(math.MaxInt32, 10),
		})
		if err != nil {
			t.Fatal(err)
		}
		if got.MaxScale != math.MaxInt32 {
			t.Fatalf("MaxScale = %d, want %d", got.MaxScale, math.MaxInt32)
		}
	})

	t.Run("max scale rejects values above int32", func(t *testing.T) {
		_, err := ResolveSettings(map[string]string{
			ClassAnnotation:    KEDAClass,
			MaxScaleAnnotation: strconv.FormatInt(int64(math.MaxInt32)+1, 10),
		})
		if err == nil {
			t.Fatal("ResolveSettings() error = nil, want max-scale overflow error")
		}
	})
