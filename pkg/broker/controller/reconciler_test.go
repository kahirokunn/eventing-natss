...
func TestReconcileFilterDeploymentReplicaPolicy(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "autoscaled preserves KEDA zero", policy: filterReplicaAutoscaled, existing: 0, wantReplicas: 0},
		{name: "fallback forces one", policy: filterReplicaFallback, existing: 0, wantReplicas: 1},
	} {
		t.Run(tc.name, func(t *testing.T) {
			existing := &appsv1.Deployment{
				ObjectMeta: metav1.ObjectMeta{Namespace: testNamespace, Name: name},
				Spec:       appsv1.DeploymentSpec{Replicas: ptr.To(tc.existing)},
			}
			kube := kubefake.NewSimpleClientset(existing)
			r := &Reconciler{
				kubeClientSet:        kube,
			...
			if err := r.reconcileFilterDeployment(testContext(), testBroker(testNamespace, testBrokerName), "TEST_STREAM", brokerconfig.DefaultBrokerConfig(), tc.policy); err != nil {
				t.Fatal(err)
			}
			got, err := kube.AppsV1().Deployments(testNamespace).Get(context.Background(), name, metav1.GetOptions{})
			if err != nil {
